...
function checkEmail(event) {
  ...
  const { reg, label, posMsg, negMsg } = inputsData.email;
...
function checkPassword1(event) {
  const el = event.target ? event.target : event;
  const { reg, label, posMsg, negMsg } = inputsData.password1;
  let regFlag =
    validateValue(reg, el.value) && el.value.length >= 8 ? true : false;
  return showMessage(el, regFlag, posMsg, negMsg, label);
}

function checkPassword2(event) {
  const el = event.target ? event.target : event;
  const { label, posMsg, negMsg } = inputsData.password2;
  let regFlag =
...
function checkCountries(event) {
  const el = event.target ? event.target : event;
  const { label, posMsg, negMsg } = inputsData.countries;
...
function checkFirstName(event) {
  const el = event.target ? event.target : event;
  const { reg, label, posMsg, negMsg } = inputsData.firstName;
  let regFlag = validateValue(reg, el.value);
  return showMessage(el, regFlag, posMsg, negMsg, label);
}
...
function checkFamilyName(event) {
  const el = event.target ? event.target : event;
  const { reg, label, posMsg, negMsg } = inputsData.familyName;
  let regFlag = validateValue(reg, el.value);
  return showMessage(el, regFlag, posMsg, negMsg, label);
}
...
function checkPosition(event) {
  const el = event.target ? event.target : event;
  const { label, posMsg, negMsg } = inputsData.position;
  let regFlag = isValueEmpty(el.value);
  return showMessage(el, regFlag, posMsg, negMsg, label);
}
...
function checkTelephone(event) {
  const el = event.target ? event.target : event;
  const { reg, label, posMsg, negMsg } = inputsData.tel;
  let regFlag = validateValue(reg, el.value);
  return showMessage(el, regFlag, posMsg, negMsg, label);
}
...
function checkCompany(event) {
  const el = event.target ? event.target : event;
  const { label, posMsg, negMsg } = inputsData.company;
  let regFlag = isValueEmpty(el.value);
  return showMessage(el, regFlag, posMsg, negMsg, label);
}
